...
如果 ZCode 自动调度出现中文输入失真、附件选择异常、登录失效、网络失败，或 Codex 无法确认 ZCode 实际收到的任务内容，Codex 必须停止自动重试，保存完整中文任务包并交由用户手动启动 ZCode。用户启动后，Codex 仍以任务文件、commit SHA、Git diff 和测试结果完成后续验收。
...
- Codex 向 Cursor、ZCode 派发任务、补充约束或反馈问题时必须使用中文；
- Cursor、ZCode 的计划、过程更新、实现交接、独立审查和回归结论必须使用中文；
- Conventional Commits 的类型前缀保留英文，冒号后的描述使用中文，例如 `fix: 拒绝纯空白的 ReAct 动作参数`；
- PR 标题、PR 正文、Review 结论、问题清单和协作复盘使用中文；
- 代码标识符、命令、文件路径、模型名、协议字段、标准名称和第三方错误原文可保留英文；
- 引用英文错误或外部资料时，应在不改变原意的前提下补充中文解释。
